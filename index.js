...
app.post("/manga", async (req, res) => {
    try {
        ...
        const response = await axios.get(`${API_URL}manga?page=1&order_by=popularity&limit=10&genres=${genreString}`);
        const result = response.data.data;
        res.render("manga.ejs", {data: result});

    } catch (error) {
        console.error("Failed to make request:", error.message);
        res.render("index.ejs", {
        error: 'There were nothing matching that query :(',
    });
    }
})
app.listen(port, () => {
